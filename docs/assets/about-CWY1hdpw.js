import"./nav-bar-crAhWqTn.js";async function p(){const l=window.location.hostname.includes("github.io"),o=window.location.hostname.includes("onrender.com"),n=l?"https://portfolio-awnu.onrender.com/data/about.json":o?"/data/about.json":"http://localhost:2000/data/about.json";try{const e=await fetch(n,{cache:"no-cache"});if(!e.ok)throw new Error(`HTTP ${e.status}`);return await e.json()}catch(e){return console.error("Failed to load about.json:",e),null}}class x extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const o=this.shadowRoot,n=document.createElement("style");n.textContent=`
...
`).forEach((d,i)=>{i&&a.appendChild(document.createElement("br")),a.appendChild(document.createTextNode(d))});const r=document.createElement("div");r.className="endorser",r.textContent=`— ${t.name||"Anonymous"}${t.role?", "+t.role:""}`,s.append(a,r),o.appendChild(s)})}}customElements.define("endorsements-list",g);class E extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const o=this.shadowRoot,n=document.createElement("style");n.textContent=`
      :host{display:block}
